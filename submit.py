...
def solve_graph(G, n, m, k, player = 1):
    start_vert = (0, 0)
    prev_vertical_edges = []
    full_path = []
    for l in range(0, n + 1):
        ...
        if l == 0:
            path = [e for e in eulerian_circuit(sub, start_vert)]
            new_full_path += path
        else:
...
def solve_graph_twice(G, n, m, k):
    path = solve_graph(G, n, m, k)

    flipped = flip_graph(G, n, m, k)

    path2 = solve_graph(flipped, n, m, k, 2)
    return (path, path2)
